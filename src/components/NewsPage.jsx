...
export const NewsPage = (props) => {
  ...
  return isFetching ? <Preloader/> : (
        <>
          <Button onClick={() => dispatch(refreshCommentsList(newIdNum))}
          ...
          {currentNews && <Card
            ...
            className={'news_body'}
            actions={[<a href={currentNews.url} target="_blank"
              rel="noreferrer">
              {currentNews.url}
            </a>]}
          >
            <Meta
              ...
              description=
                {'Количество комментариев: ' + currentNews.descendants}
            />
