...
import {
  ...
  selectAppInit,
  selectNewsList,
  selectNewsListIsFetching} from '../redux/newsSlice';
...
export const NewsCards = () => {
  ...
  if (NewsList.length !== 0) {
    NewsElements = NewsList.map((news) =>
      <Col xs={24} sm={12} md={12} lg={6} key={news?.id} span={6}>
        <NewsCard data={news}/>
      </Col>);
  }


  if (!appInitialized && NewsElements) {
    return <Skeleton title={'Loading'} active avatar paragraph={{rows: 12}}/>;
  }
  return (
    <>
      <Button onClick={() => dispatch(getNewsListAsync())}
        icon={<RetweetOutlined/>} loading={isFetching}
        type="primary">
                Обновить список
      </Button>
      <Row>
